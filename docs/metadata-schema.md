...
论文 YAML 元数据是自动索引和正文基本信息卡的唯一数据源。为避免 GitHub 将其渲染成不易读的表格，论文页必须采用 `<!--`、`-->` 包住完整 YAML front matter；脚本仍能读取，页面只展示正文。运行 `python3 scripts/sync_paper_info.py` 后，作者、机构、论文时间、期刊/会议、分类、重点标签及状态会出现在无表格的中文信息卡中。
