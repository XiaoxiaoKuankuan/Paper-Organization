...
M³GPT 的核心是把文字、人体动作和音乐都离散成 T5 可以自回归读写的 token，再用自然语言 instruction 决定当前是在生成舞蹈、配乐、描述动作还是做跨模态理解。共享 T5 负责序列推理，但动作 tokenizer 的 decoder 在跨模态预训练中还会继续联合优化，避免固定量化器限制生成质量。

### 1. 总体定位：为什么统一音乐、动作和文字比两模态更难

音乐与动作都具有长时间结构，却有不同采样率和码本；文字又是稀疏语义。若只共享 Transformer 而不对齐 token 边界，模型很难知道哪种模态该输出何种长度；若完全冻结动作 decoder，语言模型产生的 token 分布偏移会放大量化误差。M³GPT 因此使用三套 tokenizer/边界符、共享因果主干和分阶段动态解码训练。

### 2. 整体训练流程：三个阶段逐步统一

1. Stage 1 训练共享动作 tokenizer，把普通动作和舞蹈量化到同一动作词表；音乐使用预训练 Jukebox tokenizer。
2. 扩展 T5 词表，加入动作、音乐 token 及各模态开始/结束符，统一组成自回归序列。
3. Stage 2 通过文本↔动作、音乐↔舞蹈、描述与理解等任务做多模态对齐预训练，并继续调整动作 decoder 适应预测码分布。
4. Stage 3 把任务选择改写为自然语言指令，微调模型理解不同输入/输出模态组合。
5. 推理时 instruction 决定生成文字、音乐码或动作码；对应 tokenizer decoder 再恢复连续信号。

### 3. 总体信息流：三套 token、一个 T5、三个训练阶段
...
- 2026-09-04：按 ADAPT 式讲解补充三模态 token 对齐与量化误差问题，并用五步流程明确动作 tokenizer、T5 词表、对齐预训练、指令微调和解码。
