...
MACE-Dance 的核心是明确分开“舞蹈动作应该怎样编”和“指定人物应该怎样出现在视频里”。Motion Expert 先从音乐生成 SMPL 三维舞蹈，固定姿态桥把它变成二维控制，Appearance Expert 再依据参考图渲染人物视频。两个专家各自在最合适的数据上训练，最终误差不会跨过姿态提取器端到端反传。

### 1. 总体定位：为什么直接音乐生视频很难兼顾动作和身份

像素级视频模型容易保持纹理却忽略节拍，也可能动作丰富但人物身份漂移；高质量音乐—三维舞蹈和同人物多视角视频又很少同时配对。论文因此把任务拆成动作空间与外观空间：前者利用动作数据学习音乐结构，后者利用视频先验学习身份与画面，两者通过可解释的姿态控制连接，代价是中间动作或二维姿态误差会级联到最终视频。

### 2. 整体训练与推理流程：两位专家、一个固定桥

1. 将音乐特征与三维舞蹈对齐，训练 Motion Expert 的 Dance Block 和扩散主干恢复 SMPL 动作。
2. 用 Guidance-Free Training 等条件策略，让动作模型同时学习语义、节拍与无条件分布。
3. 把生成 SMPL 网格按固定相机渲染，再用 ViTPose 提取/校正二维人体控制序列。
4. 在人物视频与参考图数据上分阶段微调 Appearance Expert，使视频 DiT 同时遵循姿态并保持身份外观。
5. 推理严格按音乐→三维动作→二维姿态→参考人物视频执行；两级模型不能并行替换为一个端到端控制器。

### 3. 总体信息流：先编舞，再按人物外观渲染视频
...
- 2026-09-04：按 ADAPT 式讲解补充动作质量与身份保持的核心矛盾，并用五步级联流程明确 Motion Expert、固定姿态桥和 Appearance Expert 的训练边界。
