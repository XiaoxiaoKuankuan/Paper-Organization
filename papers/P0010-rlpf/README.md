...
RLPF 的核心不是再训练一个更强 tracker，而是把一个已经能在仿真里执行 G1 参考的冻结控制器，当作大动作模型的“物理评审员”。生成模型看不到可微动力学梯度，只收到整段动作执行后的物理与语义分数，再通过 GRPO 调整离散动作 token 的生成概率。

### 1. 总体定位：为什么文本—动作指标不够

大动作模型可以在人体数据上生成语义正确、视觉流畅的 SMPL 动作，但重定向到机器人后可能跌倒、超限或无法完成根轨迹；仅按文本相似度优化，又容易忽略物理可执行性。反过来，只奖励“不跌倒”会让模型输出站立和慢走。RLPF 因此要把不可微的机器人执行反馈、文本语义和对原动作分布的约束放进同一组相对策略优化。

### 2. 整体训练流程：从监督模型到物理后训练

1. 用 VQ tokenizer 把人体动作变成离散码，在 Motion-X 上监督微调 LLaMA 动作生成器。
2. 独立训练 Teacher tracker，再用 DAgger 蒸馏出只依赖部署观测的 Student tracker。
3. 每条文本采样 20 个动作候选，解码成人体运动并按固定契约重定向到 29 自由度 G1。
4. 冻结 Student 在 Isaac Gym 中执行每条参考，得到跟踪成功等物理奖励；冻结对比模型提供文本和参考对齐奖励。
5. 在同一提示组内标准化总奖励，用 clipped GRPO 与 KL 约束更新生成器；tracker、重定向器和仿真器都不反传梯度。

### 3. 总体信息流：把冻结控制器变成生成模型的物理评审员
...
- 2026-09-04：按 ADAPT 式讲解重构方法开篇，先解释为何文本指标不代表机器人可执行，再用五步流程讲清监督起点、Teacher/Student、仿真评分与 GRPO 更新边界。
