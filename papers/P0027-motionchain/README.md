...
MotionChain 的核心不是把每轮文本分别生成一段动作再拼接，而是把图像、文字、历史动作和当前动作都放进同一个多轮对话上下文。模型只对回答部分做因果预测，因此每一轮既能引用之前的动作，又能根据新图像或新指令继续编辑、解释或生成后续。

### 1. 总体定位：动作为什么需要“对话状态”

真实用户会先说“向前走”，再指着图片要求模仿姿势，随后要求“保持上身但转向”。单轮生成器看不到前文，简单串接动作会在边界跳变；已有多模态 LLM 又缺少动作词表。MotionChain 希望建立可持续的动作会话记忆，让不同模态提示在多轮中共同约束同一动作链，同时用历史帧与过渡数据减少段间不连续。

### 2. 整体训练流程：从动作词表到十轮对话

1. 训练 VQ 动作 tokenizer，把连续人体动作转成可由语言模型预测的离散 token。
2. 合并动作词表与 SentencePiece 文本词表，并把 CLIP 图像/视频特征投影成语言模型 embedding。
3. 用单轮文本—动作、图像—动作及理解任务做跨模态预训练，先建立基本对应关系。
4. 从动作库自动构造最多十轮的对话链，包含生成、编辑、连接、描述和跨模态引用。
5. 指令微调时把历史 source 与 answer 串成统一因果序列，只在 answer token 上计算损失。
6. 推理保留完整会话历史，动作回答经 VQ decoder 还原，并用边界历史处理连续性；输出仍是人体动作而非机器人控制。

### 3. 总体信息流：把图像、文字和动作放进多轮对话
...
- 2026-09-04：按 ADAPT 式方法导读补充多轮动作控制的上下文问题，并用六步流程明确动作/视觉 tokenizer、单轮预训练、对话构造、回答损失和连续推理。
