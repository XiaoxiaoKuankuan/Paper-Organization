...
MCM 的核心是先让一个文本条件扩散主干学会“动作应该自然且符合语义”，再像 ControlNet 一样复制出可训练控制分支，专门吸收音乐或语音的逐帧节奏。原主干冻结，控制分支每层通过零初始化 bridge 注入修正，因此新增模态在训练开始时不会立刻破坏已有文本动作先验。

### 1. 总体定位：多条件动作为什么要分主次

文本提供全局行为含义，音乐/语音提供细粒度时序，两类条件数据并不总是成对；若从头联合训练，数据量大的模态会主导网络，若只在输入端拼接，深层动作特征又可能忽略控制信号。MCM 用主分支负责通用动作分布，控制分支负责时间对齐修正，把“学会自然动作”和“遵循额外模态”分成两个阶段。

### 2. 整体训练流程：主干预训练后接入控制分支

1. 统一 HumanML3D 等动作与文本表示，编码带噪动作、扩散时刻和文本条件。
2. Stage 1 训练 MWNet 主分支，以 Multi-Wise Attention 建模时间、通道和文本关系并预测干净动作。
3. Stage 2 复制主干层形成 control branch，冻结原主干，将 Jukebox 音乐/语音特征与带噪动作输入控制支路。
4. 每层控制特征经零初始化 bridge 加到主分支对应层，只更新控制侧参数。
5. 推理按所需条件启用文本主干与音频控制，最终仍由主分支输出扩散预测；没有额外音频时退化为原文本模型。

### 3. 总体信息流：先学文本动作主干，再插入音频控制分支
...
- 2026-09-04：按 ADAPT 式讲解补充文本与音频条件的粒度冲突，并用五步流程讲清 MWNet 预训练、冻结主干、控制分支和零初始化 bridge。
