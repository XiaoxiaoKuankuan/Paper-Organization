...
LMM 的核心不是把十几种任务分别加上专用 head，而是先把异构动作库统一成 MotionVerse，再让一个扩散 Transformer 通过随机 FPS、身体/时间 mask 学习跨数据集运动先验；第二阶段才加入文本、语音、音乐、视频等条件。数据集专用 Read-In/Read-Out 只负责骨架接口，中央 ArtAttention 主干共享。

### 1. 总体定位：为什么动作“基础模型”首先是数据接口问题

不同动作数据集的骨架、帧率、关节维度、任务和条件模态各不相同，直接合并会把格式差异误当作动作规律。若为每个数据集训练独立模型，又无法共享大量无标签运动。LMM 要把输入输出适配与核心动作建模分开：先学习任何骨架上都适用的时空补全，再用统一条件 token 把同一主干微调到十类生成与编辑任务。

### 2. 整体训练流程：MotionVerse、预训练、条件化

1. 将多源数据整理为动作、可见 mask、条件和数据集标识，保留各骨架的专用 Read-In/Read-Out。
2. Stage 1 随机改变 FPS、遮蔽时间片和身体部位，在无条件扩散中恢复干净动作，学习跨库先验。
3. ArtAttention 用空间分支交流关节/部位，用时间分支建模长序列，两路在共享主干中交替更新。
4. Stage 2 将 ImageBind 等编码的文本、音频、音乐和视频投为条件 token，并做条件 dropout 训练 CFG。
5. 推理按任务设置已知动作 mask 和条件，从噪声恢复未知部分；数据集标识选择正确骨架输出层，而不是改变中央模型。

### 3. 总体信息流：统一数据、无条件预训练、条件微调
...
- 2026-09-04：按 ADAPT 式方法导读补充异构数据接口这一基础矛盾，并用五步流程讲清 MotionVerse、无条件 masked pretraining、ArtAttention 与条件微调。
