...
AvatarGPT 的核心不是只做 text-to-motion，而是把动作理解、动作生成和长任务规划都写成语言模型可以处理的序列任务。连续动作先量化为 token，图像/视频则经自动标注管线变成多粒度语言和动作监督；同一个 T5 式模型根据指令决定输出文字解释、任务步骤还是动作码。

### 1. 总体定位：为什么动作模型还需要规划与理解

传统 text-to-motion 只能把一句描述变成短动作，无法解释已有动作、预测后续、分解长任务或结合场景状态。纯 LLM 会规划文字，却不能直接产生连续可播放动作。AvatarGPT 用动作 tokenizer 和适配器把两种空间连接，使语言承担任务组织，动作 token 承担身体执行表示；长序列则通过先生成计划、再逐步生成子动作形成，而不是一次预测无限长轨迹。

### 2. 整体训练与推理流程：统一成指令到序列

1. 训练 VQ 动作 tokenizer，把连续人体动作变成离散 embedding，并保留 decoder 负责重建。
2. 用 adapter 将动作 embedding 映射到 T5 hidden space；文本仍走原 tokenizer，输出端分别保留文本 head 与动作 head。
3. 从视频和动作库自动构造描述、动作问答、预测、补全、任务分解与场景估计等多粒度数据。
4. 把所有任务写成条件序列→目标序列，联合训练/指令微调同一多模态模型。
5. 推理时先按指令生成文字计划或动作 token；动作输出经 tokenizer decoder 还原，长任务按计划逐段执行并把历史写回上下文。

### 3. 总体信息流：用语言连接规划、理解与动作生成
...
- 2026-09-04：按 ADAPT 式讲解补充动作生成与任务规划之间的缺口，并用五步流程讲清 tokenizer、adapter、双输出 head、自动标注和逐段推理。
