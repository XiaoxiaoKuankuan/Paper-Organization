...
GENMO 的核心不是简单地把多个数据集塞进一个大模型，而是让“从噪声创造动作”和“从观测恢复动作”共用同一个人体运动表示、条件接口和 Transformer 去噪主干。这样，视频估计得到的几何知识可以帮助生成，文本与音乐学到的动作先验也能反过来约束遮挡或歧义观测下的估计。

### 1. 总体定位：GENMO 要解决什么问题

传统人体动作系统往往为文本生成、音乐舞蹈、视频姿态估计、关键帧补全分别训练模型，动作坐标系、骨架、相机和损失都不一致，能力很难组合。GENMO 试图建立一个统一接口：不论条件是文本、音乐、视频、二维关键点还是部分动作，模型最终都恢复同一套带全局轨迹、相机和接触的人体运动。难点不只是模态多，而是有些任务一对多、适合扩散采样，有些任务近似确定、要求准确回归。

### 2. 整体训练流程：四条主线

1. **统一表示：** 把 SMPL 姿态、根运动、体型、相机和接触整理成同一逐帧向量，并为各数据源记录哪些字段真实可见。
2. **统一条件：** 帧对齐条件经独立 MLP 加到动作 token，分段文本通过带时间掩码的跨注意力进入主干。
3. **联合训练：** 生成样本从随机扩散时刻去噪，估计样本从最大噪声恢复唯一答案；两者共享 16 层 Transformer，并用运动学、网格、接触和重投影损失监督。
4. **组合推理：** 按任务打开相应条件和可见性 mask，从噪声或部分已知动作出发去噪，最终解码成人体动作；机器人使用还需另接重定向与跟踪器。

### 3. 总体信息流：一个主干同时做估计与生成
...
- 2026-09-04：按 ADAPT 式方法导读补充核心定位、待解决矛盾和四条训练主线，明确统一表示、条件注入、生成/估计联合训练及机器人使用边界。
