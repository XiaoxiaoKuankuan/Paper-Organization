...
Odoriko 的核心不是生成动作后再把平均体型替换成目标人物，而是在扩散去噪的每一层都让动作特征看到人体形状。带噪动作先与文本、逐帧条件和性别模板在多模态 block 中对齐，后半段再移除文本、持续注入连续 shape `β`，专门细化受体型影响的运动学关系。

### 1. 总体定位：动作与身体形状为什么不能后处理

不同身高、四肢比例和体型会改变步幅、关节位置、接触和自遮挡；先按平均身体生成再换 shape，网格可能穿插或足端漂移。视频估计时，形状又和相机、姿态互相歧义。Odoriko 要在一个主干中同时支持文本/音乐生成与视频动作估计，并让离散性别模板提供粗先验、连续 `β` 提供个体细节。

### 2. 整体训练与推理流程：条件对齐后做形状细化

1. 将动作规范化为统一 SMPL-X/相关运动表示，并整理文本、音乐、视频、相机与 subject shape 条件。
2. 编码带噪动作、逐帧局部条件、文本 token、全局条件和性别/shape token。
3. 前半段 MM Transformer blocks 联合注意动作、文本与粗形状，完成语义和时间对齐。
4. 后半段去掉文本支路，以动作、全局条件和连续 `β` 逐层调制，细化体型相关运动学。
5. 训练以扩散重建、几何和形状一致性损失共同更新；生成与视频估计使用不同条件 mask，但共享主干。
6. 推理用 UniPC 反向扩散：生成模式由用户给 shape，估计模式从视频联合恢复动作/相机；结果仍是人体模型参数。

### 3. 总体信息流：多模态去噪与人体形状在同一主干中耦合
...
- 2026-09-04：按 ADAPT 式方法结构补充形状后替换的几何问题，并用六步流程讲清多模态 blocks、分层 shape 注入、共享生成/估计主干和 UniPC 推理。
