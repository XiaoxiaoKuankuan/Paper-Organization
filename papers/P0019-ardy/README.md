...
ARDY 的核心是把用户最需要精确控制的全局根轨迹保持为显式连续量，把高维身体姿态压到 latent，再用两个扩散模型按“先根、后身体”滚动生成。它不是离线生成固定时长动作，而是在每个窗口读取可变历史和新的文本/路径/关键帧，执行前一小段后继续重规划。

### 1. 总体定位：交互动作为什么需要混合表示

全局路径、朝向和到达时间适合用明确坐标约束，身体姿态却高度多模态，直接在全维空间满足稀疏约束会让采样慢且不稳定。纯 latent 模型又难保证根轨迹精确命中。ARDY 希望同时支持在线文本变化、waypoint、完整关键帧和稀疏关节位置/旋转，因此把可控根与自然身体分开建模，并让历史窗口承担跨块连续性。

### 2. 整体训练与在线生成流程：五步

1. 训练动作 tokenizer，把身体姿态压成隐式 latent，同时保留显式根位置、朝向和速度通道。
2. 构造可变长度历史、未来文本、路径和稀疏目标 mask，让同一训练样本覆盖不同交互条件。
3. 训练 Root Diffusion 先恢复未来全局根轨迹，监督位置、朝向与连续性。
4. 将干净或预测根停止梯度地交给 Body Diffusion，生成身体 latent，再由 decoder 还原全身动作。
5. 在线推理每窗执行前段、保留尾部历史并重新读取用户条件；输出人体动作，机器人使用需另接 retargeting 和 tracker。

### 3. 总体信息流：显式根轨迹与隐式身体分级生成
...
- 2026-09-04：按 ADAPT 式方法导读补充混合表示的必要性，并用五步流程串起 tokenizer、Root/Body Diffusion、停止梯度、滚动窗口和机器人边界。
