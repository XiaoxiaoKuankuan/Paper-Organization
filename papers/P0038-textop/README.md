...
TextOp 的核心不是输入一句话后先生成完整动作再播放，而是持续维护一个运动缓冲：高层扩散模型每次依据文本和最近历史续写一个短动作块，低层通用 tracker 以更高频率读取未来参考并闭环执行。用户在中途修改提示时，新文本从下一生成块开始生效，tracker 不重启，也不需要先知道整段命令时间表。

### 1. 总体定位：实时文本控制为什么必须分成两个时钟

离线 text-to-motion 生成整段序列延迟高，动作段硬拼接会在边界跳变；直接让语言模型以 50 Hz 输出电机动作又难同时保证语义、多样性和稳定性。TextOp 把职责分层：6.25 Hz 高层处理文本和短时编舞，50 Hz tracker 处理动力学误差。两者通过机器人原生参考和 motion buffer 通信，关键是历史重叠、self-rollout 训练和生成分布增强，而不只是网络推理速度。

### 2. 整体训练与部署流程：两套数据、两个策略

1. 将人体 MoCap 经 GMR 映射到 29 自由度 G1、插值到 50 Hz，并通过特权 tracker 筛选可执行片段；BABEL 文本对齐生成数据。
2. 训练条件 VAE，把历史+未来动作压入 latent，并用机器人局部增量表示保证窗口可衔接。
3. 冻结 VAE，训练文本条件 LDM；self-rollout 逐步用模型预测历史替换真值，逼近长时滚动分布。
4. 用 AMASS、私有动作和高层生成的 31.48 小时参考训练通用 PPO tracker，使其适应生成器特有误差。
5. 部署时高层在外部 GPU 以 6.25 Hz 续写短块并放入 buffer，板载 tracker 以 50 Hz 输出关节目标。
6. 新提示只改变后续块；历史窗口与低层状态反馈维持连续，但突变命令仍可能超出动力学可衔接范围。

### 3. 总体信息流：文本滚动生成机器人动作，跟踪器闭环执行
...
- 2026-09-04：按 ADAPT 文档第一部分的结构补充双时钟系统动机，并用六步流程讲清生成数据、VAE/LDM、self-rollout、tracker 增强、motion buffer 与在线提示切换。
