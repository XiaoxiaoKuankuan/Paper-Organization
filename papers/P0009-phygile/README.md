...
PhyGile 不是单独提出一个生成器或 tracker，而是把“长尾动作怎么生成”和“单个控制器怎么跟住不同难度动作”一起设计。它先用难度课程和专家扩容训练 GMT，再用带 Token-Parameterized MoE 的扩散模型生成机器人动作，最后用 Physics-Prefix 把生成器真实会产生的误差重新送回 GMT 训练。

### 1. 总体定位：PhyGile 要解决什么问题

统一 tracker 在走路等高频样本上容易训练，却会被跳跃、翻滚和复杂上肢等长尾技能拖垮；普通 MoE 又可能路由塌缩。另一方面，文本生成器即使输出 G1 关节，也可能产生接触或连续性上不可执行的未来。论文要同时解决容量如何随动作难度增长、长尾文本如何获得专家资源，以及生成分布与跟踪训练分布不一致的问题。

### 2. 完整训练流程：三大阶段、七个动作

1. 把人体动作重定向为统一 262 维 G1 表示，并按语义/物理难度标为 12 级。
2. GMT Stage I 从易到难逐级解锁数据，每增加一级就复制并专门训练新专家。
3. GMT Stage II 混合全部可行等级，以软 top-k、负载均衡和动态扩容联合训练专家。
4. 对长尾动作做语义标注、逆频率采样和镜像增强，训练文本条件 TP-MoE 扩散生成器。
5. 从 GMT 成功执行状态截取 physics prefix，让冻结生成器续写约一秒机器人动作。
6. 在仿真中执行续写，记录生成误差和失败，而不是只在离线姿态空间筛选。
7. 保持生成器冻结，用这批分布内/边界样本继续 PPO 微调 GMT，缩小生成—跟踪差距。

### 3. 总体信息流：先训练可扩展跟踪器，再训练机器人生成器
...
- 2026-09-04：按 ADAPT 式方法讲解补充统一 tracker、长尾生成和分布错位三类问题，并用三阶段七步流程明确 GMT 课程、TP-MoE 与 Physics-Prefix 的先后关系。
