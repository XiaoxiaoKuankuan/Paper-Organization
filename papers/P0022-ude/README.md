...
UDE 的核心不是让一个网络直接从文本或音乐回归连续动作，而是把任务拆成“统一条件理解—离散动作规划—连续细节恢复”。文本和音乐先经 MATE 对齐到共享语义空间，UTT 自回归生成动作 token，Diffusion Motion Decoder 再以整条 token 序列为骨架恢复连续动作，从而让两种模态共享高层动作词表而保留各自时间特征。

### 1. 总体定位：为什么统一生成需要四个模块

文本描述长时语义，音乐提供密集节拍，二者与动作的对齐粒度不同；若直接混合原始条件，模型容易为每种模态形成独立通路。只生成离散 token 又会受到量化误差限制。UDE 因此把条件表示、动作词表、序列规划和连续细化分开训练：先解决“说的是同一种动作吗”，再决定“动作码按什么顺序出现”，最后补回量化中丢失的姿态细节。

### 2. 整体训练顺序：四个模块依次建立

1. 训练 Motion Quantization，将连续动作编码为离散 token，并让基础 decoder 可重建动作。
2. 训练 MATE，把文本全局语义和音乐逐帧特征映射到可与动作对齐的条件表示。
3. 冻结/固定前级表示，训练 Unified Token Transformer 根据条件自回归预测动作码序列。
4. 以完整 token 序列和条件为输入训练 Diffusion Motion Decoder，在连续空间恢复量化丢失的细节。
5. 推理沿条件编码→token 规划→扩散细化顺序执行；文本与音乐共享动作生成骨架，但不能假设两者的条件时间分辨率相同。

### 3. 总体信息流：条件对齐、离散规划、连续细化
...
- 2026-09-04：按 ADAPT 式讲解补充统一文本/音乐生成的表示矛盾，并用五步顺序明确 Motion Quantization、MATE、UTT 与连续扩散细化的职责和冻结关系。
