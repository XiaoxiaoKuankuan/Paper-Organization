...
AnyMo 的核心不是为文本、音频、轨迹或关键帧各训练一个控制分支，而是把所有已知条件都视为“可见 token”，把要生成的动作视为“被遮住的 token”。模型通过双向 masked prediction 反复补全空位，因此同一个 Transformer 可以处理任意模态组合、动作补全和局部编辑；前提是先有大规模对齐数据和能逐层补细节的动作 tokenizer。

### 1. 总体定位：任意模态生成的两个上游瓶颈

多模态动作数据规模小且标签不齐，模型即使结构统一也没有足够组合可学；单层 VQ 又会在压缩动作时丢失手部和快速细节。AnyMo 先用 OmniHuMo 把网络视频自动恢复为动作、文本、音频和轨迹，再用 Residual-FSQ 把量化误差逐层编码。这样 masked Transformer 看到的是统一、可扩展的离散空间，而不是彼此长度和精度不同的原始模态。

### 2. 整体训练流程：数据、tokenizer、生成器三阶段

1. OmniHuMo 对互联网视频做人检、跟踪、三维恢复、接触/质量筛选，并生成分层文本与同步音频条件。
2. 训练 R-FSQ tokenizer：第一层编码主体动作，后续层继续量化前层残差，decoder 汇总各层恢复连续运动。
3. 把文本、音频、轨迹、关键点和动作码投到共享 token 空间，并记录时间/身体位置 mask。
4. 先进行单模态和大规模 masked pretraining，再逐步加入任意条件组合与高质量数据微调。
5. 推理从全 mask 或部分已知动作开始，按置信度迭代填充 token；用户约束保持可见，剩余位置由模型补全。
6. 输出是人体动作而非机器人控制，物理可执行性仍需单独验证。

### 3. 总体信息流：大规模自动标注、残差量化与并行补全
...
- 2026-09-04：按 ADAPT 式方法结构补充任意模态生成的数据与量化瓶颈，并用六步流程讲清 OmniHuMo、R-FSQ、共享 token、分阶段训练和 mask-predict 推理。
