...
GigaBrain-WBC-0.5 所说的“行为世界模型”不是预测相机画面的通用 world model，而是让控制器在输出下一关节动作的同时，预测下一本体状态和下一行为潜变量分布。这个分布随后用于判断外部参考是否偏离训练经验，并在真正送入控制器前把异常行为条件投影回较可信区域。

### 1. 总体定位：它要解决什么问题

大规模动作跟踪器可以模仿丰富参考，但部署时上游生成器、遥操作或地形误差会给出训练分布外命令。普通 tracker 只能被动跟随，直到状态明显失稳；硬裁剪关节又无法理解完整行为是否异常。论文希望建立一个仍保持 50 Hz 的因果控制器：既学习动作和地形交互，也利用自身历史预测“下一步什么行为条件合理”，在 OOD 参考进入前做轻量修正。

### 2. 整体训练与部署流程：五个阶段

1. 从多源动作建立未来 10 帧参考，并用行为编码器压成 64 维连续条件。
2. 从低速接触和人体几何自动恢复支撑原语，生成动作对应的交互地形。
3. 在平地与地形环境中用 PPO 训练因果 Transformer 输出 29 维 PD 目标。
4. 同时训练下一状态头、行为 GMM 头、参考重建和循环一致性，让隐状态具备预测能力。
5. 部署时用上一时刻 GMM 检查当前行为条件；超出概率椭球时先投影，再由控制器执行并进入下一反馈周期。

### 3. 总体信息流：行为编码、因果控制与在线修正
...
- 2026-09-04：按 ADAPT 式讲解补充“行为世界模型”的准确定位、OOD 参考问题和五阶段训练/部署链，先建立因果关系再进入维度与损失细节。
