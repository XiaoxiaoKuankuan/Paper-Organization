...
这篇论文不是再提出一个名称更复杂的 tracker，而是把通用动作跟踪里经常一起改变的设计拆开做受控实验。它先固定 Unitree G1、动作库、奖励、随机化和 PPO 预算，再一次只改变参考速度、历史长度、动作参数化、PD 增益、外力交互或训练范式，用相同指标判断什么真正带来收益。

### 1. 总体定位：它为什么要做“消融型论文”

不同跟踪论文往往同时更换网络、数据、奖励、观测和执行器，最终成功率提高却无法知道原因；某些复杂技巧也可能只在特定 PD 增益或测试集上有效。作者要建立的是一套可复用的实证结论：哪些输入对闭环跟踪必要，哪些常见模块收益有限，如何按机器人机械参数设置动作尺度，以及单阶段 PPO 是否已经足以替代复杂 Teacher–Student 管线。

### 2. 整体研究流程：固定基线，再逐项改变

1. 将 AMASS/OMOMO 统一重定向到 29 自由度 G1，建立训练和留出动作集。
2. 固定基准策略的本体观测、未来参考、奖励、域随机化、并行环境和训练预算。
3. 分组改变参考速度、历史、残差/绝对动作、PD 增益、外力交互和 Teacher–Student 等因素。
4. 在相同成功率、姿态/根误差、恢复和交互协议下比较，并检查因素之间是否耦合。
5. 把胜出配置重新组合成最终 tracker，验证单项结论在完整系统中是否仍成立；部署时只保留最终 actor 和物理可用观测。

### 3. 总体信息流：用受控实验拆解跟踪器设计
...
- 2026-09-04：按 ADAPT 式讲解补充实证研究的必要性和五步受控实验流程，明确固定项、单变量消融、组合验证与部署结论之间的逻辑。
