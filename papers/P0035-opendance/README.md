...
OpenDance 的核心不是只发布一个更大的舞蹈数据集，也不是只换一个 masked Transformer，而是把数据恢复、动作表示、条件生成和推理后修正连成闭环。它先把网络视频整理成带音乐、文本、二维/三维信息的 OpenDanceSet，再把旋转、二维关键点和全局轨迹分开量化，最后联合补全三路 token 并对脚滑做显式梯度修正。

### 1. 总体定位：开放域舞蹈生成为什么要同时改数据和模型

棚拍舞蹈数据动作干净但风格有限，网络视频规模大却有相机、遮挡、尺度和接触噪声；单一三维动作表示又难同时保留局部姿态、画面构图和世界轨迹。若模型只优化生成指标，脚接触错误仍会在长舞蹈中累积。OpenDance 因此将三种互补表示和后优化纳入完整系统，而不是把所有责任交给生成主干。

### 2. 整体流程：数据构建到推理修正六步

1. 从约 600 小时网络视频检测、跟踪和筛选舞蹈，恢复人体、相机、音乐及分层文本，得到约 100.26 小时 OpenDanceSet。
2. 对恢复动作做后优化、接触与质量筛选，明确区分原始视频时长和最终训练时长。
3. 训练 Disentangled Dance Tokenizer，分别量化关节旋转、二维关键点和全局轨迹。
4. Multimodal-Condition Transformer 随机遮蔽三路 token，在音乐、文本和已有动作条件下联合恢复。
5. 推理用 MaskGIT 式迭代按置信度填充，使不同控制条件可固定相应 token。
6. MS-LRM 结合足接触和局部重建，在连续空间用梯度修正脚滑与轨迹误差。

### 3. 总体信息流：建库、三路量化、联合掩码、逐步修正
...
- 2026-09-04：按 ADAPT 式方法导读补充开放域数据、三路表示和接触误差的联动问题，并用六步流程讲清数据构建、DDT、MCT、MaskGIT 与 MS-LRM。
