...
UDE-2 的核心增量不是简单给 UDE 多加语音输入，而是同时解决“不同条件模态”和“不同身体部位”两种异质性。躯干、左手、右手分别量化以保留不同运动尺度，文本、音乐、语音各自编码后进入共享 Transformer；基础 decoder 学公共时序，各部位/模态 head 再预测对应动作 token。

### 1. 总体定位：为什么全身动作不能只用一个码本

躯干位移幅度大、节奏慢，手指动作幅度小、变化快；单一码本容易优先重建身体而丢失手部。文本、音乐和语音的条件粒度也不同，直接统一会把共语手势、舞蹈和日常动作混成一个平均分布。UDE-2 需要让各部位保留专用容量，又让它们在同一时间轴上协调，并让三种条件共享动作语义而不抹去模态特点。

### 2. 整体训练流程：分部位量化、共享主干生成

1. 分别训练躯干、左手和右手 tokenizer，建立三套动作词表与 decoder。
2. 用预训练文本、音乐和语音 encoder 提取条件，经 adapter 统一到共享 hidden space。
3. 共享 Transformer encoder 融合条件，causal base decoder 读取历史动作 token 建模公共时序。
4. 部位/模态专用 head 预测下一 token，并用语义增强目标在 latent 层对齐条件与动作。
5. 推理按任务选择条件编码器和 head，逐步采样三路 token，再分别解码并合成为同步全身动作。
6. 输出仍是人体动作；手部词表和条件同步正确不代表机器人手或全身动力学可直接执行。

### 3. 总体信息流：按身体部位量化，再统一条件生成
...
- 2026-09-04：按 ADAPT 式方法导读补充身体部位与条件模态的双重异质性，并用六步流程讲清分部位 tokenizer、共享 Transformer、专用 head 与全身恢复。
