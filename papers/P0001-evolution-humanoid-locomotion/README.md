...
这篇文章最容易被读成一份按年份排列的文献清单，但它真正要建立的是一张“任务意图怎样一路变成电机执行量”的控制地图。阅读时不要先记 MPC、WBC、RL 或生成模型的名字，而要先问：谁产生未来参考、谁处理接触与动力学、谁在高频闭环纠错，以及每层使用的是模型约束、数据先验还是奖励反馈。

### 1. 总体定位：这篇综述到底在梳理什么问题

人形机器人运动控制长期存在三类彼此纠缠的问题：动力学和接触约束复杂，纯模型方法难以覆盖全部误差；强化学习能吸收复杂经验，但奖励、仿真分布和安全边界不透明；生成模型能给出丰富动作，却未必知道机器人能否执行。论文的核心不是判定哪一类方法“最好”，而是解释这些方法分别位于控制栈的哪一层、解决哪一种不确定性，以及为什么今天的强系统通常要把它们组合起来。

### 2. 阅读全文时先记住四层信息流

1. **任务层：** 用户命令、目标速度、地形、文本或场景观测定义机器人想完成什么。
2. **参考层：** 步态规划、轨迹优化、动作库或生成模型产生落脚点、质心轨迹、全身参考或技能潜变量。
3. **控制层：** MPC、WBC、模仿策略或强化学习策略把参考和当前状态变成关节位置、速度或力矩。
4. **执行层：** PD、电机、接触动力学和状态估计形成真实反馈，再把误差送回上层。综述中的每种技术都应放回这四层判断，而不是只按网络结构分类。
...
- 2026-09-04：按 ADAPT 式讲解顺序重构方法导读，先解释综述要解决的分类问题，再用任务、参考、控制和执行四层信息流组织后续技术路线。
