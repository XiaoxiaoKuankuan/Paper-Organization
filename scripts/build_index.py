"""
从论文 YAML 元数据生成知识库首页统计、最近更新和分类索引。
...
输出：稳定排序的 Markdown 索引；相同输入会得到相同输出。脚本不读取或复制
``local_archive`` 中的受版权约束材料，也不会执行任何 Git 操作。
"""
...
def build_outputs(root: Path, records: list[PaperRecord]) -> dict[Path, str]:
    ...
    ordered = sorted(records, key=lambda item: str(item.metadata.get("id", item.directory.name)))
    papers_index = "\n\n".join(
    ...
    for slug, title in CATEGORIES.items():
        ...
        category_text = "\n\n".join(
            [f"# {title}", GENERATED_NOTICE, paper_table(root, selected, root / "index")]
        ) + "\n"
